...
image::ContactSails_Interface_Breakdown.png[width="500" align="center"]
...
We are planning to add the ability to only view orders specific to a person in `v2.0`.
...
In Figure 19, the calendar is set to Day-view, hence it displays all calendar entries that occur in the displayed date (11 April 2018, Wednesday).
In the calendar, calendar entries appears as colored blocks, such as the green block in Figure 19, indicating the title and starting time of the entry.
...
Figure 21 below shows how a calendar entry appears in the calendar entry list.

.Calendar Entry Description
image::Calendar_Entry_Card.png[width]

In Figure 21, calendar entry has an index of 1, indicating its position in the list.
...
image::Edit_Entry_Before.png[width="200" align="center"]
...
image::Edit_Entry_After.png[width="200" align="center"]
