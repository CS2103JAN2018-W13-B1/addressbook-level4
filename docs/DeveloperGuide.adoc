...
Figure 11 shows the inheritance diagram for commands.
...
`CenterPanel` subsribes to `BaseEvents` related to the `CalendarPanel` and `PersonPanel`, such as `ChangeCalendarPageRequestEvent` and `PersonPanelSelectionChangedEvent`.
...
In Figure ?, `ViewTodayCommand` posts a new `ChangeCalendarPageRequestEvent` to the `EventsCenter`.
...
.. Incorrect `calendarjump` commands to try:
    ...
    ** `calendarjump 04.04.2018` (Calendar will display in Day-view if no parameter is given) +
