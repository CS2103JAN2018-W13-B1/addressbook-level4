...
`Order` objects are kept in-memory during an application session with the help of a `UniqueOrderList` object. The `UniqueOrderList` object ensures that
...
Currently, ContactSails supports the `dark` and `light` themes.
...
When the `ChangeThemeCommand` is executed, a `ChangeThemeEvent` is raised by `EventsCenter`. The `handleChangeThemeEvent() method in the `MainWindow`
...
theme version it receives as a parameter.
