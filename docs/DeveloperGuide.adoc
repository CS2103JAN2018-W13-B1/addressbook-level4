...
//tag::Person Panel[]
=== Person Panel
The `PersonPanel` replaces the previous `BrowserPanel`, which is called upon using the methods in `CenterPanel`.
`PersonPanel` will be used to display contact's information, groups and preferences depending on user's actions.

==== Layout Implementation
The specifications for the layout of PersonPanel is in `PersonPanel.fxml`. The figure below will show the visual layout of the `PersonPanel`.
...
==== Current Implementation
By utilising `java.util.logging` package, PersonPanel is able to display contact's particulars, its
groups and preferences through selecting its respective `PersonCard`.
...
----
Whenever a contact is selected, an event is triggered. Once the event is triggered,
